...
const addMedicine = asyncHandler(async (req, res) => {
    const { name, weight, cost, field, aka, details } = req.body;
    // Choose an alternative default image URL
    const defaultImageUrl = 'https://source.unsplash.com/featured/?medicine,pill';
    const by = "Doctor Sahab";
    try {
        if (!name || !weight || !cost || !field) {
            res.status(400);
        ...
        // Save the medicine with the chosen default image URL
        const medicine = await new Medic({ name, aka, weight, cost, field, image: defaultImageUrl, details, by }).save();
        res.status(200).json(medicine);
    } catch (error) {
        res.status(400);
        throw new Error(error);
